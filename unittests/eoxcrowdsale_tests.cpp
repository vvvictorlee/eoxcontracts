...
#include <eoxcrowdsale/eoxcrowdsale.wast.hpp>
#include <eoxcrowdsale/eoxcrowdsale.abi.hpp>
...
class eoxcrowdsale_tester : public tester {
public:

   eoxcrowdsale_tester() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol), N(eoxcrowdsale) } );
      produce_blocks( 2 );

      set_code( N(eoxcrowdsale), eoxcrowdsale_wast );
      set_abi( N(eoxcrowdsale), eoxcrowdsale_abi );

      produce_blocks();

    //   const auto& accnt = control->db().get<account_object,by_name>( N(eoxcrowdsale) );
    //   abi_def abi;
...
//       vector<char> data = get_row_by_account( N(eoxcrowdsale), symbol_code, N(stat), symbol_code );
...
//       auto symb = eosio::chain::symbol::from_string(symbolname);
//       auto symbol_code = symb.to_symbol_code().value;
//       vector<char> data = get_row_by_account( N(eoxcrowdsale), acc, N(accounts), symbol_code );
...
//       return push_action( N(eoxcrowdsale), N(create), mvo()
//            ( "issuer", issuer)
...
//    }
   ...
   action_result buytokens(  account_name to,
                  uint64_t    _value ) {
      return push_action( N(eoxcrowdsale), N(buytokens), mvo()
           ( "_beneficiary", to)
           ( "value", _value)
           );
   }
...
BOOST_AUTO_TEST_SUITE(eoxcrowdsale_tests)

// BOOST_FIXTURE_TEST_CASE( create_tests, eoxcrowdsale_tester ) try {
...
// BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, eoxcrowdsale_tester ) try {
...
// BOOST_FIXTURE_TEST_CASE( symbol_already_exists, eoxcrowdsale_tester ) try {
...
// BOOST_FIXTURE_TEST_CASE( create_max_supply, eoxcrowdsale_tester ) try {
...
// BOOST_FIXTURE_TEST_CASE( create_max_decimals, eoxcrowdsale_tester ) try {
...
// BOOST_FIXTURE_TEST_CASE( issue_tests, eoxcrowdsale_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, eoxcrowdsale_tester ) try {
