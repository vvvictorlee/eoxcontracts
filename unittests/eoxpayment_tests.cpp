...
#include <eoxpayment/eoxpayment.wast.hpp>
#include <eoxpayment/eoxpayment.abi.hpp>
...
class eoxpayment_tester : public tester {
public:

   eoxpayment_tester() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol), N(eoxpayment) } );
      produce_blocks( 2 );

      set_code( N(eoxpayment), eoxpayment_wast );
      set_abi( N(eoxpayment), eoxpayment_abi );

      produce_blocks();

    //   const auto& accnt = control->db().get<account_object,by_name>( N(eoxpayment) );
    //   abi_def abi;
...
//       vector<char> data = get_row_by_account( N(eoxpayment), symbol_code, N(stat), symbol_code );
...
//       auto symb = eosio::chain::symbol::from_string(symbolname);
//       auto symbol_code = symb.to_symbol_code().value;
//       vector<char> data = get_row_by_account( N(eoxpayment), acc, N(accounts), symbol_code );
...
//       return push_action( N(eoxpayment), N(create), mvo()
//            ( "issuer", issuer)
...
//    }
   ...
   action_result ptransfer(  account_name to,
                  uint64_t    _value ) {
      return push_action( N(eoxpayment), N(transfer), mvo()
           ( "_dest", to)
           ( "_amount", _value)
           );
   }
...
BOOST_AUTO_TEST_SUITE(eoxpayment_tests)

// BOOST_FIXTURE_TEST_CASE( create_tests, eoxpayment_tester ) try {
...
// BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, eoxpayment_tester ) try {
...
// BOOST_FIXTURE_TEST_CASE( symbol_already_exists, eoxpayment_tester ) try {
...
// BOOST_FIXTURE_TEST_CASE( create_max_supply, eoxpayment_tester ) try {
...
// BOOST_FIXTURE_TEST_CASE( create_max_decimals, eoxpayment_tester ) try {
...
// BOOST_FIXTURE_TEST_CASE( issue_tests, eoxpayment_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, eoxpayment_tester ) try {
